...
GameBoyAdvanceSoftwareRenderer.prototype.freeze = function() {
	var palette = new Uint16Array(0x200);
	for (var i = 0; i < 0x100; ++i) {
		palette[i] = this.palette.colors[0][i] || 0;
		palette[i + 0x100] = this.palette.colors[1][i] || 0;
	}
	return {
		'palette': palette.buffer,
		'vram': this.vram.buffer.buffer.slice(0),
		'oam': this.oam.buffer.buffer.slice(0)
	};
};
