...
GameBoyAdvanceInterruptHandler.prototype.updateTimers = function() {
	...
	if (this.timersEnabled) {
		...
		timer = this.timers[1];
		if (timer.enable) {
			// Count-up timers do not have an independent clock event.  A zero
			// nextEvent means that the preceding timer has not overflowed yet.
			if (timer.nextEvent && this.cpu.cycles >= timer.nextEvent) {
				timer.lastEvent = timer.nextEvent;
				timer.nextEvent += timer.overflowInterval;
