...
function ROMView(rom, offset) {
	...
	this.PAGE_MASK = (2 << this.ICACHE_PAGE_BITS) - 1;
	// Each MMU region exposes at most 16 MiB.  Smaller cartridges are
	// mirrored throughout that window by the GBA bus, so their mask must be
	// based on the actual ROM view instead of the maximum 32 MiB cart size.
	// Keeping the old 0x01FFFFFF mask made 4/8 MiB games read past DataView,
	// and left the upper half of 16 MiB games without an instruction cache.
	this.mask = Math.min(this.view.byteLength, 0x01000000) - 1;
	this.resetMask();
	this.icache = new Array(Math.max(1, (this.mask + 1) >> (this.ICACHE_PAGE_BITS + 1)));
};
...
GameBoyAdvanceMMU.prototype.loadRom = function(rom, process) {
	if (!rom || rom.byteLength < 0xC0 || rom.byteLength > this.SIZE_CART0) {
		return null;
	}

	// Commercial cartridges use power-of-two capacities.  Pad odd-sized
	// homebrew/bad dumps to the next capacity so masked reads and DMA cannot
	// escape the backing DataView.
	var normalizedSize = 1;
	while (normalizedSize < rom.byteLength) {
		normalizedSize <<= 1;
	}
	if (normalizedSize != rom.byteLength) {
		var normalizedRom = new ArrayBuffer(normalizedSize);
		new Uint8Array(normalizedRom).set(new Uint8Array(rom));
		rom = normalizedRom;
	}
	...
	var hi = rom.byteLength > 0x01000000 ? new ROMView(rom, 0x01000000) : lo;
